...
int SERVOMIN[]  {104, 100, 108}; //PLEASE ENTER: The lower motor PPM limit (Servo Min) as noted in your EDMO box
int SERVOMAX[]  {444, 442, 510}; //PLEASE ENTER: The upper motor PPM limit (Servo Max) as noted in your EDMO box

int POTIMIN[]  {141, 119, 231}; //PLEASE ENTER: The lower potentiometer limit (Poti Low) as noted in your EDMO box
int POTIMAX[]  {717, 684, 521}; //PLEASE ENTER: The upper potentiometer limit (Poti High) as noted in your EDMO box
...
//CPG parameter
double frequency = 0.4; // oscillator frequency
double rateOfFrequency = 0;
double targetFrequency = 0.4;

double w = 2; // we assume that all oscillators use same coupling weight
...
{
//values for bi-module control  
//    {0,17,17,70,70,0,0,0,0,0,{0,PI/2.},{0,1}},
//    {0,45,45,150,150,0,0,0,0,0,{-PI/4.,0},{1,0}}


//values for tri-module control
      {0,20,20,20,20,0,0,0,0,0,{0,PI/3,0},{0,1,0}},
      {0,0,0,80,80,0,0,0,0,0,{-PI/3,0,PI/3},{1,0,1}},
      {0,20,20,20,20,0,0,0,0,0,{0,PI/3, 0},{0,1,0}}
...
void loop(){   
    ...
    if (currentMillis - previousMillis >= timeStep){//CPG update interval (10ms)(do NOT modify!!)
      ...
      for (int i = 0; i < NUM_OSCILLATORS; i++) {
        ...
        osc[i].rateOfPhase = 2 * PI * frequency;

        for (int j = 0; j< NUM_OSCILLATORS ; j++) {
          if (i != j) {
            osc[i].rateOfPhase += w * osc[j].amplitude * sin(osc[j].phase - osc[i].phase - osc[i].phaseBias[j]);
          }
        }

     
        osc[i].rateOfAmplitude = a * (osc[i].targetAmplitude - osc[i].amplitude);
        osc[i].amplitude = osc[i].amplitude + (osc[i].rateOfAmplitude * dt);
        osc[i].phase = osc[i].phase + (osc[i].rateOfPhase * dt);
        ...
        osc[i].offset = osc[i].offset + (osc[i].rateOfOffset * dt);
      ...
      if (rateOfFrequency != 0) {
        frequency = frequency + (rateOfFrequency * dt);
      }
